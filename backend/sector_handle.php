<?php session_start();
    ...
    if (file_exists($target_file)) {
        header('Location:add_sectors.php?choice=error&value=Sorry File Already Exist');
        exit();
    }
    ...
    if ($_FILES["thumbnail"]["size"] > 500000000) {
        header('Location:add_sectors.php?choice=error&value=Sorry File too Large ');
        exit();
    }
    ...
    && $imageFileType != "gif" && $imageFileType != "webp" && $imageFileType != "WEBP"  && $imageFileType != "pdf") {
        header('Location:add_sectors.php?choice=error&value=Sorry, only JPG, JPEG, PNG and GIF files are allowed.');       
        exit();
    }
    if($imageFileType1 != "jpg" && $imageFileType1 != "png" && $imageFileType1 != "jpeg"
    && $imageFileType1 != "gif" && $imageFileType != "webp" && $imageFileType != "WEBP"   && $imageFileType1 != "pdf") {
        header('Location:add_sectors.php?choice=error&value=Sorry, only JPG, JPEG, PNG and GIF files are allowed.');       
        exit();
    }
    
    if ($uploadOk == 0) {
          header('Location:add_sectors.php?choice=error&value=Sorry, your File was Not Uploaded');
         exit();
    } else {
        if (move_uploaded_file($_FILES["thumbnail"]["tmp_name"], $target_file)) {
          ...
          $table = "sector";
           $product_images = implode(",", $arryt);
             $key_list = "`menu_name`, `sector_title`, `service_category`, `summery`, `property_desc`,`table_img`,  `thumbnail`, `property_images`, `status`";
              $value_list  = "'".$_REQUEST['menu_name']."',";
              $value_list .= "'".$_REQUEST['sector_title']."',";
              $value_list .= "'".clean($_REQUEST['service_category'])."',";
